    ...
    <section className="py-24 md:py-32 bg-ivory relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 md:px-8 relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-20">
          ...
          <h2 className="font-heading text-display-sm font-bold text-primary mb-6">{t('storiesHeading')}</h2>
          <p className="text-lg text-muted max-w-2xl mx-auto">{t('storiesSub')}</p>
        </motion.div>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {testimonials.map((testimonial, index) => (
            <motion.div key={testimonial.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: index * 0.1 }} className="group">
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.5 }} className="text-center mt-12">
          <a href="/testimonials" className="inline-flex items-center gap-2 text-primary font-semibold hover:text-accent transition-colors">
            {t('storiesReadMore')} <Star className="w-5 h-5" />
          </a>
        </motion.div>
      </div>
    </section>
