    ...
    <section className="py-16 md:py-24 lg:py-32 bg-sand relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 md:px-8 relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-12">
          ...
          <h2 className="font-heading text-2xl md:text-3xl lg:text-display-sm font-semibold text-primary mb-4">{t('sigHeading')}</h2>
          <p className="text-base md:text-lg text-muted max-w-2xl mx-auto">{t('sigSub')}</p>
        </motion.div>
        ...
        <div className="hidden lg:block relative h-[600px] mb-8">
          {experiences.map((exp, index) => (
            <motion.div key={exp.id} initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: index * 0.1 }}
              ...
              <Link to={exp.link}>
                <div className="w-[360px] h-[500px] rounded-3xl overflow-hidden shadow-lift hover:shadow-glass-strong transition-all duration-500 hover:-translate-y-2 hover:scale-105">
